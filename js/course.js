(function () {
  ...
  const periodChoices = [
    [
      { value: "underload", text: "ITPH ниже плана. Проанализировать прошлые периоды: разовое снижение — перерывы, подготовка и чистота; повторное — корректировка часов без ущерба следующей смене" },
      { value: "low-cut-now", text: "ITPH ниже плана. Отпустить сотрудника домой и сократить часы смены, опираясь на текущий период, отсутствие очереди и прогноз потока ресторана до конца дня", feedback: "Один час ниже плана ещё не подтверждает устойчивое снижение. Сначала проанализируй прошлые периоды и ожидаемый поток." },
      { value: "low-prep-all-shift", text: "ITPH ниже плана. Сохранить все часы команды и до конца смены направить свободных сотрудников на подготовку, чистоту и пополнение запасов по плану ресторана", feedback: "Подготовка полезна при разовом снижении, но решение на всю смену нельзя принимать без анализа прошлых периодов." },
      { value: "low-transfer-now", text: "ITPH ниже плана. Перенаправить сотрудника в другой ресторан до конца смены, ориентируясь на текущий поток, ожидаемый пик и запрос соседней команды на этот день", feedback: "Перенаправление возможно при повторяющемся снижении, но сначала нужно проверить динамику и не оставить следующую смену без помощи." }
    ],
    [
      { value: "overload", text: "ITPH выше плана. Наблюдать 2–5 минут, определить западающую станцию и усилить её опытным сотрудником" },
      { value: "high-call-now", text: "ITPH выше плана. Вызвать дополнительного сотрудника и поставить его на кухню для общего усиления команды в пик", feedback: "Дополнительный сотрудник может понадобиться, но сначала определи западающую станцию и пойми, поможет ли он устранить задержку." },
      { value: "high-manager-station", text: "ITPH выше плана. Управляющему занять самую загруженную станцию до конца пика, а команде сохранить текущую расстановку", feedback: "Помощь управляющего даст временный эффект, но не устранит причину. Сначала понаблюдай и скорректируй расстановку команды." },
      { value: "high-speed-up", text: "ITPH выше плана. Сохранить расстановку и поставить команде цель повысить темп работы на текущих станциях", feedback: "Команда уже перегружена. Просьба ускориться без поиска западающей станции повышает риск ошибок и не устраняет причину очереди." }
    ],
    [
      { value: "quality", text: "ITPH соответствует плану, но есть жалобы. Разобрать конкретный отзыв, найти корневую причину, исправить её и проверить результат" },
      ...
      { value: "quality-add-person", text: "ITPH соответствует плану, но есть жалобы. Добавить сотрудника на кухню для снижения нагрузки, а отзывы разобрать после пика", feedback: "ITPH и скорость соответствуют плану. Дополнительный сотрудник не исправит проблему, пока не определена причина жалоб." }
    ],
    [
      ...
      { value: "stable-no-check", text: "ITPH соответствует плану, скорость и качество в норме. Сохранить расстановку и перенести следующую проверку на завершение смены", feedback: "ITPH нужно контролировать каждый час, чтобы вовремя заметить изменение потока, скорости или качества." },
    ...
    [
      ...
      { value: "high-manager-counter", text: "ITPH выше плана, ожидание растёт. Управляющему занять выдачу и сосредоточиться на скорости передачи заказов Гостям", feedback: "Работа на выдаче не устранит неизвестную причину задержки. Сначала найди западающую станцию и усиль её." }
    ]
  ];
