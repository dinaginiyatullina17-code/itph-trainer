(function () {
  ...
  const periodChoices = [
    [
      ...
      { value: "low-cut-now", text: "ITPH ниже плана. Сразу отпустить сотрудника домой и сократить часы смены, ориентируясь только на этот период и отсутствие очереди, без анализа прошлых периодов", feedback: "Один час ниже плана ещё не подтверждает устойчивое снижение. Сначала проанализируй прошлые периоды и ожидаемый поток." },
      { value: "low-prep-all-shift", text: "ITPH ниже плана. Сохранить все часы команды и до конца смены занять свободных сотрудников подготовкой и чистотой, не анализируя динамику прошлых периодов", feedback: "Подготовка полезна при разовом снижении, но решение на всю смену нельзя принимать без анализа прошлых периодов." },
      { value: "low-transfer-now", text: "ITPH ниже плана. Сразу перенаправить сотрудника в другой ресторан до конца смены, не проверяя ожидаемый пик, прошлые периоды и потребности следующей смены", feedback: "Перенаправление возможно при повторяющемся снижении, но сначала нужно проверить динамику и не оставить следующую смену без помощи." }
    ],
    [
      { value: "overload", text: "ITPH выше плана. Наблюдать 2–5 минут, определить западающую станцию и усилить её опытным сотрудником" },
      { value: "high-call-now", text: "ITPH выше плана. Сразу вызвать дополнительного сотрудника, не определяя причину задержки и загрузку станций", feedback: "Дополнительный сотрудник может понадобиться, но сначала определи западающую станцию и пойми, поможет ли он устранить задержку." },
      { value: "high-manager-station", text: "ITPH выше плана. Менеджеру самому встать на самую загруженную станцию до конца пика, не меняя расстановку", feedback: "Помощь менеджера даст временный эффект, но не устранит причину. Сначала понаблюдай и скорректируй расстановку команды." },
      { value: "high-speed-up", text: "ITPH выше плана. Сохранить расстановку и попросить всю команду работать быстрее, не определяя западающую станцию", feedback: "Команда уже перегружена. Просьба ускориться без поиска западающей станции повышает риск ошибок и не устраняет причину очереди." }
    ],
    [
      ...
      { value: "quality-add-person", text: "ITPH соответствует плану, но есть жалобы. Добавить сотрудника на кухню и снизить нагрузку, не разбирая конкретные отзывы", feedback: "ITPH и скорость соответствуют плану. Дополнительный сотрудник не исправит проблему, пока не определена причина жалоб." }
    ],
    [
